# coding: utf-8
# The plain model contains 2 parts of information:
# 1. Category Embedding;
# 2. Other features in the dataset.
import pandas as pd
import json
...
train_data = train_set.merge(business_features, on='business_id', how='left').merge(user_features, on='user_id', how='left').drop(['user_id', 'business_id'], axis=1).fillna(0)
eval_data = eval_set.merge(business_features, on='business_id', how='left').merge(user_features, on='user_id', how='left').drop(['user_id', 'business_id'], axis=1).fillna(0)
test_data = test_set.merge(business_features, on='business_id', how='left').merge(user_features, on='user_id', how='left').drop(['user_id', 'business_id'], axis=1).fillna(0)
...
for i, (train_index, test_index) in enumerate(cv.split(X)):
    ...
    model = xgb.XGBRegressor(n_estimators=350, max_depth=7, learning_rate=0.07, tree_method='gpu_hist')
    ...
    pickle.dump(model, open(f"./models/plain/model_{i}_xgbt.pkl", 'wb'))
...
for i, (train_index, test_index) in enumerate(cv.split(X)):
    ...
    pickle.dump(model, open(f"./models/plain/model_{i}_cat.pkl", 'wb'))
